...
export default function PriorityFilter({
  ...
  const { data } = useSelector((state) => state.priorities);
  ...
  const toggleFilter = (priority) => {
    setCurrentFilter((prev) => {
      const exists = prev.some((item) => item.id === priority.id);
      if (exists) {
        return prev.filter((item) => item.id !== priority.id);
      } else {
        return [...prev, { id: priority.id, name: priority.name }];
      }
    });
  };
